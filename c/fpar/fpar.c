...
char **get_tokens ()
{
  ...
  term_tok = (int**)malloc(sizeof(int*)*termnum);
  term_tok_num = (int*)malloc(sizeof(int)*termnum);
  memset (term_tok, 0, sizeof(int*)*termnum);
  memset (term_tok_num, 0, sizeof(int)*termnum);

  /* how many tokens are there? */
  for (i=0; op[i].syntax!=NULL; i++) {
    j=count_tokens (op[i].syntax);
    term_tok[i]=(int*)malloc(sizeof(int)*2*(j+1));
		     ...
		     int *error)
{
...
#if DEBUG==2
  fprintf (stderr, "depth %i (%s): %s\n", depth, op[depth].syntax, *s);
#endif
  ...
  if (op[depth].syntax==NULL) { /* then it must be a numerical value */
...
#ifdef DEBUG
      fprintf (stderr, "ERROR: unexpected non-NUM_TOK.\n");
#endif
    ...
    *oldtok = fpar_lexer (s, oldval);
    ...
    if (*oldtok==NO_TOK) {
#ifdef DEBUG
      fprintf (stderr, "ERROR: no token found at beginning of \"%s\".\n", *s);
#endif
  ...
  if (*oldtok==term_tok[depth][starti]) { /* yes! */
    do {
      i=starti;
#ifdef DEBUG
      fprintf (stderr, "MATCH: Operation %i recognized.\n", depth);
#endif
      res=(node_t*)malloc(sizeof(node_t));
      memset (res, 0, sizeof(node_t));
      res->type = depth;
      res->childs = (node_t**)malloc(term_tok_num[depth]*sizeof(node_t*));
      memset (res->childs, 0, term_tok_num[depth]*sizeof(node_t*));
...
#ifdef DEBUG
            /* there seems to be a token at the wrong place */
	    fprintf (stderr, "ERROR: parseerror in depth %i\n", depth);
#endif
...
#ifdef DEBUG
      fprintf (stderr, "ERROR: no remaining operations before %s.\n", 
               (*s)-strlen(tokens[(*oldtok >=0 ? *oldtok : 0)]));
#endif
      *error = FPAR_ERR_NOOP;
      return NULL;
    }
...
void free_node_tree (node_t *n)
{
  ...
  if (n==NULL) return;
  if (n->childs)
    for (i=0; n->childs[i]; i++) free_node_tree (n->childs[i]);
  free(n);
  return;
}
...
void tree_output (node_t *n, int currpos)
{
  ...
  printf ("%*s", TREEOUT_WIDTH, op[n->type].syntax);
  currpos+=TREEOUT_WIDTH;
  for (i=0; n->childs[i]; i++) {
    if (i!=0) printf ("%*s", currpos, " ");
