...
function VideoAdapter(playerId, cbAPIFunc, defaultVideoName, defaultVideoUsername, doReloadPlayer) {
	...
	// Gets an unique name for the function which will be called when the player API is ready
	var apiFuncName = getUniqueFunctionName("ytfplayerReady");

	// Makes sure the player API is enabled (needs a player reload)
	this.setFlashVar("enablejsapi", "1", false);

	// Saves the original YouTube initialization function name and sets the player to call our function when the API is ready (needs a player reload)
	var originalApiCbFuncName = this.getFlashVar("jsapicallback") || "onYouTubePlayerReady"; // "onYouTubePlayerReady" is the default function name if "jsapicallback" isn't set or is an empty string
	this.setFlashVar("jsapicallback", apiFuncName, false);

	// Function that is called by the player when the API is ready
	// Calls the original YouTube initialization function and the API-dependant features function
	unsafeWindow[apiFuncName] = function(apiPlayerId) {

		// The original YouTube initialization function is called if it exists
		if (unsafeWindow[originalApiCbFuncName]) unsafeWindow[originalApiCbFuncName](apiPlayerId);

		// Calls the API-dependant features function
		videoAdapterAPI();

	};

	//'
	function videoAdapterAPI() {
...
var scriptVersion = 1364175359867; // 25 Mar 2013
...
function scriptWatchMain() {
	...
	function resizePlayer(sizeValue, doScroll) {
		...
		if (playlistBar) { // If the playlist exists it is resized too to match the player
			playlistDiv.style.paddingLeft = videoDiv.style.paddingLeft;
			playlistBar.style.width = playerDiv.style.width;
		}
