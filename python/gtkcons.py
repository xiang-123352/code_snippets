...
class Console(gtk.VBox):
    def __init__(self, namespace={}, quit_cb=None):
        ...
        self.inputbox.pack_end(self.closer, expand=False)
        self.closer.show()
    ...
    def run(self, cmd):
        ...
        self.prompt.queue_draw()
        gobject.idle_add(self.scroll_bottom)
        sys.stdout, self.stdout = self.stdout, sys.stdout
        sys.stderr, self.stderr = self.stderr, sys.stderr

def gtk_console(ns, title='Python', menu=None, quit_cb=None):
    win = gtk.Window()
    ...
    win.set_title(title)
    cons = Console(namespace=ns, quit_cb=quit_cb)
    if menu:
        box = gtk.VBox()
        win.add(box)
        box.show()
